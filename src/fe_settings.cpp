...
const std::string &FeSettings::get_tempo_ocioso() const
{
	return m_ocioso;
}
...
const std::string &FeSettings::get_current_display_mode() const
{
	if ( m_current_display < 0 )
		return m_menu_prompt; // When showing the 'Displays Menu', title=configured menu prompt

	return m_displays[m_current_display].get_info( FeDisplayInfo::InMode );
}
