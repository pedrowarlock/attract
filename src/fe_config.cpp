...
void FeDisplayEditMenu::get_options( FeConfigContext &ctx )
{
	...
	if ( display )
	{
		...
		std::vector<std::string> coin_opts( 2 );
		ctx.fe_settings.get_resource( "FICHA", coin_opts[0] );
		ctx.fe_settings.get_resource( "TEMPO", coin_opts[1] );
		
		ctx.add_optl( Opt::LIST, "Modo cobrança",
			display->show_in_mode() ? coin_opts[0] : coin_opts[1],
			"_help_display_in_mode" );
		ctx.back_opt().append_vlist( coin_opts );
	...
	if ( display )
	{
		for ( int i=0; i< FeDisplayInfo::LAST_INDEX; i++ )
		{
			if (( i == FeDisplayInfo::InCycle )
				|| ( i == FeDisplayInfo::InMenu ) || ( i == FeDisplayInfo::InMode ))
			{
				display->set_info( i,
...
void FeMiscMenu::get_options( FeConfigContext &ctx )
{
	...
	ctx.add_optl( Opt::EDIT,
			"Default Time Name",
			ctx.fe_settings.get_info( FeSettings::TimeName ),
			"" );	

	ctx.add_optl( Opt::EDIT,
			"Credito Por Fichas",
			ctx.fe_settings.get_info( FeSettings::CreditoPorFicha ),
			"" );
	ctx.add_optl( Opt::EDIT,
			"Tempo Por Fichas",
			ctx.fe_settings.get_info( FeSettings::TempoPorFicha ),
			"" );
	
	ctx.add_optl( Opt::EDIT,
			"Ocioso",
			ctx.fe_settings.get_info( FeSettings::Ocioso ),
			"" );
