         ...
         const std::string &value, const std::string &fn )
{
	...
	if ( setting.compare( indexStrings[Layout] ) == 0 ) // layout
		...
		m_info[ InMenu ] = value;
	else if ( setting.compare( indexStrings[InMode] ) == 0 ) // in_mode
		m_info[ InMode ] = value;
...
void FeDisplayInfo::save( nowide::ofstream &f ) const
{
	...
	f << '\t' << setw(20) << left
		<< indexStrings[InMode] << ' ' << get_info( InMode ) << endl;
...
bool FeDisplayInfo::show_in_mode() const
{
	return config_str_to_bool( m_info[InMode] );
}
